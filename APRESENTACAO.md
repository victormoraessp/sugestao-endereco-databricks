...
### Bloco 5b. O efeito se sustenta ao longo do ano

**Título de ação**
A vantagem do cadastro se mantém mês a mês, e a melhora não vem de outra causa.

**Subtítulo de ação**
A entrega sem o telefone recomendado é a linha de base, e é contra ela que o ganho precisa ser medido.

**Texto de apoio**

Uma análise agregada de doze meses pode esconder duas coisas opostas. O cadastro pode estar envelhecendo entre as cargas, com a vantagem encolhendo mês a mês. Ou a entrega pode estar melhorando sozinha, por higienização de base, mudança de fornecedor ou perfil de campanha, e nesse caso parte do ganho observado não seria mérito do Golden Record.

O bloco H do relatório separa as duas. A tendência da vantagem, na linha 96, diz se o cadastro está ganhando ou perdendo poder de discriminação, em pontos percentuais por mês. A tendência da entrega no braço divergente, na linha 97, é a linha de base da operação sem o cadastro.

A leitura conjunta define a conclusão. Se as duas séries sobem juntas, a melhora é da operação e o valor do Golden Record precisa ser apresentado com essa ressalva. Se apenas a vantagem sobe, o cadastro está ganhando qualidade. Se a vantagem cai enquanto a base sobe, o cadastro está envelhecendo e a periodicidade de carga precisa ser revista.

Tendência mensal da vantagem: **[linha 96]**. Tendência da entrega sem o telefone recomendado: **[linha 97]**. Meses em que o Golden Record ficou à frente: **[linha 97, campo de exemplo]**.

**Visual**

```mermaid
flowchart TD
    S["Serie mensal<br/>de 2025"]
    S --> V["Tendencia da vantagem<br/>linha 96"]
    S --> B["Tendencia sem o cadastro<br/>linha 97"]
    V --> C1["Sobe: cargas recentes<br/>acrescentam informacao"]
    V --> C2["Cai: cadastro envelhece,<br/>rever periodicidade"]
    B --> C3["Sobe junto: parte da melhora<br/>nao e do cadastro"]
    B --> C4["Estavel: o ganho<br/>e todo do cadastro"]
    style S fill:#e8eef7,stroke:#33475b
    style C2 fill:#fff6e5,stroke:#b8860b
    style C3 fill:#fff6e5,stroke:#b8860b
    style C4 fill:#e6f4ea,stroke:#1e7e34
```
