/* =============================================================================
   ...
   indicador por linha. O bloco E conta a história do valuation por briefing em
   cinco etapas e cita os briefings que explicam cada número. O detalhe linha a
   linha por briefing fica disponível na view vt_res_10_briefing_valuation.
...
/* -----------------------------------------------------------------------------
   ...
   REGRA DE RESERVA
     ...
     ausentes desta tabela recebem os valores gerais, e o resumo informa quanto
     do valor total vem de briefings com parâmetro próprio.

   COMO PREENCHER
     Uma linha por briefing dentro do VALUES. As três linhas abaixo são
     exemplos reais de formato e podem ser mantidas, substituídas ou
     complementadas. Os identificadores são escritos entre aspas porque a
...
SELECT cc.grupo, 'D. Valuation pelo custo de disparo', 36 + cc.ordem,
  ...
  CASE cc.cenario
    ...
    WHEN 'EVIDENCIA_EXTRAPOLADA' THEN 'Cenario de estresse, detalhado na linha 72 do bloco E. Costuma ficar abaixo dos demais e pode ficar negativo.'
...
/* ---------------------------------------------------------------- BLOCO E -- */
UNION ALL
SELECT a.grupo, 'E. Valuation pela conversao', 50, 'Briefings analisados',
  CAST(a.briefings AS DOUBLE), 'briefings',
  'Quantidade de briefings do escopo. Sao os briefings listados na secao 00.5, cada um com taxa de conversao e ticket proprios. Briefing e campanha sao um para um.',
  'Contagem de briefings com economia informada.',
  'E o universo inteiro do estudo. Tudo que vem abaixo e a soma desses briefings, cada um convertido pela propria economia, sem nenhuma premissa unica aplicada ao total.',
  concat('Briefings e economias: ', d.lista_economia, '.'),
  'vt_res_10_briefing_destaques'
FROM vt_res_10_conversao_atual a JOIN vt_res_10_briefing_destaques d ON a.grupo = d.grupo
UNION ALL
SELECT v.grupo, 'E. Valuation pela conversao', 51, 'Entregas atuais nos briefings',
  CAST(v.entregas AS DOUBLE), 'entregas',
  'SMS que chegaram ao destino nos briefings do escopo, com o telefone que o CRM usou.',
  'Soma das entregas de todos os briefings.',
  ...
  concat('O briefing de maior volume e o ', d.brf_maior_volume, ', com ',
         format_number(d.entregas_maior_volume, 0), ' entregas.'),
  'vt_res_04_volume'
FROM vt_res_04_volume v JOIN vt_res_10_briefing_destaques d ON v.grupo = d.grupo
UNION ALL
SELECT a.grupo, 'E. Valuation pela conversao', 52, 'Conversoes estimadas hoje',
...
FROM vt_res_10_conversao_atual a JOIN vt_res_10_briefing_destaques d ON a.grupo = d.grupo
UNION ALL
SELECT a.grupo, 'E. Valuation pela conversao', 53, 'Receita estimada hoje',
...
FROM vt_res_10_conversao_atual a JOIN vt_res_10_briefing_destaques d ON a.grupo = d.grupo
UNION ALL
SELECT a.grupo, 'E. Valuation pela conversao', 54, 'Taxa de conversao efetiva',
...
FROM vt_res_10_conversao_atual a JOIN vt_res_10_briefing_destaques d ON a.grupo = d.grupo
UNION ALL
SELECT a.grupo, 'E. Valuation pela conversao', 55, 'Ticket medio efetivo',
...
UNION ALL
SELECT s.grupo, 'E. Valuation pela conversao', 56, 'Candidatos a recuperacao',
  CAST(s.candidatos AS DOUBLE), 'envios',
  'SMS que falharam nos briefings do escopo e para os quais o Golden Record indicaria um telefone diferente do usado.',
  'Insucessos com cobertura do Golden Record cujo numero acionado difere do telefone de ranking 1.',
  ...
  concat('Sao ', format_number(100.0 * s.candidatos / nullif(an.insucessos, 0), 1),
         ' por cento de todos os insucessos dos briefings.'),
  'vt_res_07_saldo'
FROM vt_res_07_saldo s
JOIN vt_res_04_anatomia_insucesso an ON s.grupo = an.grupo
WHERE s.cenario = 'CALIBRADO_CAMPANHA'
UNION ALL
SELECT s.grupo, 'E. Valuation pela conversao', 57, 'Entregas recuperadas, cenario central',
  round(CAST(s.recuperadas AS DOUBLE), 0), 'entregas',
  ...
  concat('Taxa efetiva de recuperacao de ', format_number(100.0 * s.taxa_aplicada, 2),
         ' por cento. O briefing ', d.brf_maior_recuperadas, ' e o que mais recupera, com ',
         format_number(d.maior_recuperadas, 0), ' entregas.'),
  'vt_res_07_saldo'
FROM vt_res_07_saldo s JOIN vt_res_10_briefing_destaques d ON s.grupo = d.grupo
WHERE s.cenario = 'CALIBRADO_CAMPANHA'
UNION ALL
SELECT s.grupo, 'E. Valuation pela conversao', 58, 'Entregas atuais em risco',
  CAST(s.em_risco AS DOUBLE), 'entregas',
  'SMS que hoje sao entregues com um numero diferente do recomendado e que a adocao substituiria.',
  'Entregas com cobertura do Golden Record cujo numero acionado difere do telefone de ranking 1.',
  ...
  concat('Equivalem a ', format_number(100.0 * s.em_risco / nullif(v.entregas, 0), 1),
         ' por cento das entregas atuais dos briefings.'),
  'vt_res_07_saldo'
FROM vt_res_07_saldo s JOIN vt_res_04_volume v ON s.grupo = v.grupo
WHERE s.cenario = 'CALIBRADO_CAMPANHA'
UNION ALL
SELECT s.grupo, 'E. Valuation pela conversao', 59, 'Entregas perdidas, cenario central',
  round(CAST(s.perdidas AS DOUBLE), 0), 'entregas',
...
WHERE s.cenario = 'CALIBRADO_CAMPANHA'
UNION ALL
SELECT s.grupo, 'E. Valuation pela conversao', 60, 'Entregas liquidas, cenario central',
  round(CAST(s.entregas_liquidas AS DOUBLE), 0), 'entregas',
  'Ganho liquido de entregas ao adotar o telefone recomendado: recuperadas menos perdidas.',
  'Linha 57 menos linha 59.',
  'E o numero fisico que o Golden Record entrega, antes de qualquer conta financeira. Tudo que vem a seguir e a traducao deste saldo em conversoes e receita, briefing a briefing.',
...
WHERE s.cenario = 'CALIBRADO_CAMPANHA'
UNION ALL
SELECT s.grupo, 'E. Valuation pela conversao', 61, 'Lift na taxa de entrega',
  round(CAST(100.0 * s.pp_incremental AS DOUBLE), 4), 'p.p.',
  'Quanto a taxa de entrega dos briefings subiria com a adocao, em pontos percentuais.',
  'Entregas liquidas divididas pelos disparos com cobertura do Golden Record.',
...
UNION ALL
SELECT cv.grupo, 'E. Valuation pela conversao', 62, 'Conversoes incrementais, cenario central',
...
UNION ALL
SELECT cv.grupo, 'E. Valuation pela conversao', 63, 'Receita incremental anual, cenario central',
  round(CAST(cv.receita_incremental AS DOUBLE), 2), 'R$',
  'Receita adicional que o Golden Record produziria no ano, somando o ganho de cada briefing convertido pela economia daquele briefing.',
  'Para cada briefing, conversoes incrementais multiplicadas pelo ticket do proprio briefing. Depois, soma.',
  'E o numero central do valuation e o que deve ir para a decisao. Nao depende de premissa unica: cada briefing entra com a taxa e o ticket que a operacao informou para ele. A faixa de cenarios nas linhas 67 a 71 delimita a incerteza de premissa em torno deste valor.',
...
WHERE cv.cenario = 'CALIBRADO_CAMPANHA'
UNION ALL
SELECT cv.grupo, 'E. Valuation pela conversao', 64, 'Crescimento sobre a receita atual',
...
WHERE cv.cenario = 'CALIBRADO_CAMPANHA'
UNION ALL
SELECT d.grupo, 'E. Valuation pela conversao', 65, 'Briefings em que o Golden Record reduziria a receita',
...
UNION ALL
SELECT d.grupo, 'E. Valuation pela conversao', 66, 'Receita incremental com adocao seletiva',
...
UNION ALL
SELECT cv.grupo, 'E. Valuation pela conversao', 66 + cv.ordem,
  concat('Receita incremental anual, cenario ', cv.cenario),
  round(CAST(cv.receita_incremental AS DOUBLE), 2), 'R$',
...
UNION ALL
SELECT ev.grupo, 'E. Valuation pela conversao', 72,
  'Entrega do telefone recomendado entre clientes em que ele ja foi acionado',
  round(CAST(100.0 * ev.taxa_historica AS DOUBLE), 4), '%',
  'Taxa de entrega do telefone recomendado, restrita aos candidatos para os quais esse numero ja foi acionado alguma vez.',
...
UNION ALL
SELECT ev.grupo, 'E. Valuation pela conversao', 73, 'Candidatos sem historico do telefone recomendado',
  round(CAST(100.0 * ev.fracao_sem_historico AS DOUBLE), 4), '%',
  'Fracao dos candidatos cujo telefone recomendado nunca foi acionado para aquele cliente.',
...
FROM vt_res_07_evidencia ev
UNION ALL
SELECT sen.grupo, 'E. Valuation pela conversao', 74, 'Receita incremental com metade da conversao',
...
UNION ALL
SELECT sen.grupo, 'E. Valuation pela conversao', 75, 'Receita incremental com o dobro da conversao',
  round(CAST(sen.receita_incremental AS DOUBLE), 2), 'R$',
...
UNION ALL
SELECT fe.grupo, 'E. Valuation pela conversao', 76, 'Receita incremental, limite inferior estatistico',
  round(CAST(fe.receita_inf AS DOUBLE), 2), 'R$',
  'Piso da receita incremental considerando a margem de erro da propria medicao.',
  'Limite inferior do intervalo de 95 por cento da diferenca bruta, aplicado ao volume com cobertura e ao valor efetivo de cada entrega.',
...
UNION ALL
SELECT fe.grupo, 'E. Valuation pela conversao', 77, 'Receita incremental, limite superior estatistico',
  round(CAST(fe.receita_sup AS DOUBLE), 2), 'R$',
  'Teto da receita incremental considerando a margem de erro da propria medicao.',
  'Limite superior do intervalo de 95 por cento da diferenca bruta, aplicado ao volume com cobertura e ao valor efetivo de cada entrega.',
...
FROM vt_res_11_faixa_estatistica fe
UNION ALL
SELECT a.grupo, 'E. Valuation pela conversao', 78, 'Valor de cada ponto percentual de entrega',
...
/* -----------------------------------------------------------------------------
   ...
   O bloco E usa as mesmas colunas para contar sua história como funil:
   entregas atuais sobre disparos, candidatos sobre insucessos, recuperadas
   sobre candidatos, entregas em risco sobre entregas atuais, perdidas sobre
   entregas em risco, e o saldo líquido sobre os disparos com cobertura.
----------------------------------------------------------------------------- */
...
WITH contexto AS (
  SELECT
    ...
    CASE r.ordem
      ...
      WHEN  50 THEN 'contagem absoluta, sem denominador'
      WHEN  51 THEN 'todos os disparos dos briefings'
      WHEN  52 THEN 'entregas atuais, convertidas pela taxa de cada briefing'
      WHEN  53 THEN 'conversoes atuais, convertidas pelo ticket de cada briefing'
      WHEN  54 THEN 'entregas atuais, media ponderada das taxas dos briefings'
      WHEN  55 THEN 'conversoes atuais, media ponderada dos tickets dos briefings'
      WHEN  56 THEN 'apenas os insucessos dos briefings'
      WHEN  57 THEN 'apenas os candidatos a recuperacao'
      WHEN  58 THEN 'apenas as entregas atuais dos briefings'
      WHEN  59 THEN 'apenas as entregas atuais em risco'
      WHEN  60 THEN 'disparos com cobertura do Golden Record'
      WHEN  61 THEN 'disparos com cobertura do Golden Record'
      WHEN  62 THEN 'entregas liquidas, convertidas pela taxa de cada briefing'
      WHEN  63 THEN 'conversoes incrementais, convertidas pelo ticket de cada briefing'
      WHEN  64 THEN 'receita atual dos briefings'
      WHEN  65 THEN 'contagem absoluta, sem denominador'
      WHEN  66 THEN 'briefings com saldo positivo no cenario central'
      WHEN  72 THEN 'apenas os candidatos com historico do telefone recomendado'
      WHEN  73 THEN 'todos os candidatos a recuperacao'
      WHEN  74 THEN 'receita incremental do cenario central'
      WHEN  75 THEN 'receita incremental do cenario central'
      WHEN  76 THEN 'intervalo de 95 por cento da diferenca, sobre o volume com cobertura'
      WHEN  77 THEN 'intervalo de 95 por cento da diferenca, sobre o volume com cobertura'
      WHEN  78 THEN 'um centesimo dos disparos com cobertura de cada briefing'
      ...
      ELSE CASE
        ...
        WHEN r.ordem BETWEEN 67 AND 71 THEN 'entregas liquidas do cenario, somadas briefing a briefing'
    ...
    CASE r.ordem
      ...
      WHEN 51 THEN CAST(c.total_entregas AS DOUBLE)
      WHEN 56 THEN CAST(c.candidatos_recuperacao AS DOUBLE)
      WHEN 57 THEN CAST(c.entregas_recuperadas AS DOUBLE)
      WHEN 58 THEN CAST(c.entregas_em_risco AS DOUBLE)
      WHEN 59 THEN CAST(c.entregas_perdidas AS DOUBLE)
      WHEN 60 THEN CAST(c.entregas_liquidas AS DOUBLE)
      ELSE NULL
    END                                                      AS quantidade,
    CASE r.ordem
      ...
      WHEN 51 THEN CAST(c.total_disparos AS DOUBLE)
      WHEN 56 THEN CAST(c.total_insucessos AS DOUBLE)
      WHEN 57 THEN CAST(c.candidatos_recuperacao AS DOUBLE)
      WHEN 58 THEN CAST(c.total_entregas AS DOUBLE)
      WHEN 59 THEN CAST(c.entregas_em_risco AS DOUBLE)
      WHEN 60 THEN CAST(c.disparos_com_cobertura AS DOUBLE)
      ELSE NULL
    ...
    CASE r.ordem
      ...
      WHEN  9 THEN CAST(c.total_disparos AS DOUBLE)
      WHEN 51 THEN CAST(c.total_disparos AS DOUBLE)
      WHEN 56 THEN CAST(c.total_disparos AS DOUBLE)
      WHEN 57 THEN CAST(c.total_disparos AS DOUBLE)
      WHEN 58 THEN CAST(c.total_disparos AS DOUBLE)
      WHEN 59 THEN CAST(c.total_disparos AS DOUBLE)
      WHEN 60 THEN CAST(c.total_disparos AS DOUBLE)
      ELSE NULL
