...
-- =====================================================================================
-- 2. SÓCIOS NORMALIZADOS + TODAS AS HIPÓTESES COMO COLUNAS
--    (o Databricks não aceita função SQL dentro do LATERAL VIEW/inline, então todas as
--     hipóteses, inclusive as de DV recalculado, são calculadas aqui como colunas comuns)
-- =====================================================================================
CREATE OR REPLACE TEMPORARY VIEW vw_socio_doc_01_base AS
SELECT
  y.*,
  -- CPF = base(9) + controle(2)
  CASE WHEN b_num AND len_b <= 9 AND c RLIKE '^[0-9]{1,2}$'
       THEN concat(lpad(b, 9, '0'), lpad(c, 2, '0')) END                         AS h_cpf_base_ctrl,
  -- CPF com DV recalculado (controle nulo ou digitado errado)
  CASE WHEN b_num AND len_b <= 9
       THEN concat(lpad(b, 9, '0'), fn_cpf_dv(lpad(b, 9, '0'))) END              AS h_cpf_base_dvcalc,
  -- CPF inteiro dentro do campo base
  CASE WHEN b_num AND len_b BETWEEN 10 AND 11 THEN lpad(b, 11, '0') END          AS h_cpf_base_completo,
  -- CNPJ = raiz(8) + filial(4) + controle(2)
  CASE WHEN len_b <= 8 AND length(f) <= 4 AND c RLIKE '^[0-9]{1,2}$'
       THEN concat(lpad(b, 8, '0'), lpad(f, 4, '0'), lpad(c, 2, '0')) END        AS h_cnpj_raiz_filial_ctrl,
  -- CNPJ matriz (filial ausente -> 0001) + controle
  CASE WHEN len_b <= 8 AND f_zero AND c RLIKE '^[0-9]{1,2}$'
       THEN concat(lpad(b, 8, '0'), '0001', lpad(c, 2, '0')) END                 AS h_cnpj_raiz_0001_ctrl,
  -- CNPJ com DV recalculado (filial informada ou 0001)
  CASE WHEN len_b <= 8 AND coalesce(length(f), 0) <= 4
       THEN concat(cnpj_b12, fn_cnpj_dv(cnpj_b12)) END                          AS h_cnpj_raiz_filial_dvcalc,
  -- CNPJ com base de 9 dígitos significativos (descarta o 1º) - improvável
  CASE WHEN len_b = 9 AND length(f) <= 4 AND c RLIKE '^[0-9]{1,2}$'
       THEN concat(right(b, 8), lpad(f, 4, '0'), lpad(c, 2, '0')) END            AS h_cnpj_base9_trunc,
  -- CNPJ inteiro dentro do campo base
  CASE WHEN len_b BETWEEN 12 AND 14 THEN lpad(b, 14, '0') END                    AS h_cnpj_base_completo,
  -- Concatenação crua dos 3 campos, do jeito que vieram
  CASE WHEN length(raw_concat) IN (11, 14) THEN raw_concat END                   AS h_raw_concat
FROM (
  SELECT
    x.*,
    length(b)                                                AS len_b,
    ...
    concat(lpad(b, 8, '0'), lpad(CASE WHEN f IS NULL OR f = '0' THEN '1' ELSE f END, 4, '0')) AS cnpj_b12,
    concat(coalesce(b_raw, ''), coalesce(f_raw, ''), coalesce(c_raw, ''))                    AS raw_concat,
...
-- =====================================================================================
-- 3. EMPILHA AS HIPÓTESES + VALIDAÇÃO DE DV
--    prior = plausibilidade estrutural (0-10), só serve de desempate
--    (dentro do inline só há colunas e expressões nativas, nenhuma função SQL)
-- =====================================================================================
...
WITH h AS (
  ...
  LATERAL VIEW inline(array(
    named_struct('hipotese', 'CPF_BASE_CTRL',           'tipo', 'CPF',  'doc', s.h_cpf_base_ctrl,
                 'dv_reconstruido', false, 'prior', CASE WHEN s.f_zero THEN 10 ELSE 4 END),
    named_struct('hipotese', 'CPF_BASE_DVCALC',         'tipo', 'CPF',  'doc', s.h_cpf_base_dvcalc,
                 'dv_reconstruido', true,  'prior', CASE WHEN s.c IS NULL AND s.f_zero THEN 6 ELSE 1 END),
    named_struct('hipotese', 'CPF_BASE_COMPLETO',       'tipo', 'CPF',  'doc', s.h_cpf_base_completo,
                 'dv_reconstruido', false, 'prior', 5),
    named_struct('hipotese', 'CNPJ_RAIZ_FILIAL_CTRL',   'tipo', 'CNPJ', 'doc', s.h_cnpj_raiz_filial_ctrl,
                 'dv_reconstruido', false, 'prior', CASE WHEN s.f_zero THEN 3 ELSE 10 END),
    named_struct('hipotese', 'CNPJ_RAIZ_0001_CTRL',     'tipo', 'CNPJ', 'doc', s.h_cnpj_raiz_0001_ctrl,
                 'dv_reconstruido', false, 'prior', 5),
    named_struct('hipotese', 'CNPJ_RAIZ_FILIAL_DVCALC', 'tipo', 'CNPJ', 'doc', s.h_cnpj_raiz_filial_dvcalc,
                 'dv_reconstruido', true,  'prior', CASE WHEN s.c IS NULL THEN 5 ELSE 1 END),
    named_struct('hipotese', 'CNPJ_BASE9_TRUNC',        'tipo', 'CNPJ', 'doc', s.h_cnpj_base9_trunc,
                 'dv_reconstruido', false, 'prior', 2),
    named_struct('hipotese', 'CNPJ_BASE_COMPLETO',      'tipo', 'CNPJ', 'doc', s.h_cnpj_base_completo,
                 'dv_reconstruido', false, 'prior', 5),
    named_struct('hipotese', 'RAW_CONCAT',
                 'tipo', CASE length(s.h_raw_concat) WHEN 11 THEN 'CPF' WHEN 14 THEN 'CNPJ' END,
                 'doc', s.h_raw_concat,
...
g AS (
  SELECT
    ...
    max(nome_chave_pf)                                    AS nome_chave_pf,
    max(nome_chave_pj)                                    AS nome_chave_pj,
...
WITH cand AS (
  SELECT * FROM vw_socio_doc_02_candidatos WHERE dv_valido      -- DV inválido nunca existe na RF
),
m_pf AS (
  SELECT c.*,
...
WITH pend AS (
  ...
  FROM vw_socio_doc_01_base s
  LEFT JOIN vw_socio_doc_04_vencedor v
...
WITH j AS (
  ...
  FROM vw_socio_doc_01_base s
  LEFT JOIN vw_socio_doc_04_vencedor  v ON v.cclub = s.cclub AND v.cseq_quadr_empr = s.cseq_quadr_empr
  LEFT JOIN vw_socio_doc_05_repescagem r ON r.cclub = s.cclub AND r.cseq_quadr_empr = s.cseq_quadr_empr
),
k AS (
  SELECT j.*,
