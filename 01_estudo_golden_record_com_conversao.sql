/* =============================================================================
   ...
   SAÍDA
   Dois resultados são exibidos. O primeiro é o resumo executivo, na seção
   12.2, com um indicador por linha. O segundo é o detalhe por briefing, na
   seção 12.3, com uma linha por briefing ordenada pela receita incremental, que
   responde diretamente onde o Golden Record recupera receita. Todos os
   cálculos intermediários ficam disponíveis como views para auditoria.
   ...
   ESTRUTURA
     ...
     11  Aprofundamentos
     12  Resumo executivo e detalhe por briefing
...
/* -----------------------------------------------------------------------------
   12.3  DETALHE POR BRIEFING

   Segunda saída, complementar ao resumo. Uma linha por briefing no cenário
   central, ordenada da maior para a menor receita incremental. É a resposta
   direta a onde o Golden Record recupera receita, e a lista de prioridade para
   a implantação em ondas.

   Leitura das colunas:
     receita_atual            o que o briefing já gera com as entregas de hoje
     entregas_liquidas        entregas recuperadas menos entregas perdidas
     receita_incremental      quanto o briefing ganharia, com a própria economia
     crescimento_receita      receita incremental sobre receita atual
     participacao_acumulada   quanto do ganho positivo total já foi acumulado
                              até este briefing, na ordem decrescente
     origem_taxa e origem_ticket  se a economia veio da tabela de parâmetros ou
                              da reserva do grupo
   Briefings com receita incremental negativa aparecem ao final e são os casos
   em que a adoção deve ser seletiva.
----------------------------------------------------------------------------- */
SELECT
  v.grupo,
  v.id_briefing,
  v.n_envios                                                   AS disparos,
  v.n_entregues                                                AS entregas_atuais,
  v.n_insucessos                                               AS insucessos,
  round(CAST(v.recuperadas AS DOUBLE), 0)                      AS entregas_recuperadas,
  round(CAST(v.perdidas AS DOUBLE), 0)                         AS entregas_perdidas,
  round(CAST(v.entregas_liquidas AS DOUBLE), 0)                AS entregas_liquidas,
  round(CAST(100.0 * v.taxa_conversao AS DOUBLE), 4)           AS taxa_conversao_pct,
  round(CAST(v.ticket_medio AS DOUBLE), 2)                     AS ticket_medio,
  v.origem_taxa,
  v.origem_ticket,
  round(CAST(v.receita_atual AS DOUBLE), 2)                    AS receita_atual,
  round(CAST(v.conversoes_incrementais AS DOUBLE), 1)          AS conversoes_incrementais,
  round(CAST(v.receita_incremental AS DOUBLE), 2)              AS receita_incremental,
  round(CAST(100.0 * v.crescimento_receita AS DOUBLE), 4)      AS crescimento_receita_pct,
  round(CAST(v.custo_desperdicado AS DOUBLE), 2)               AS custo_em_insucesso,
  round(CAST(v.midia_requalificada AS DOUBLE), 2)              AS midia_requalificada,
  r.posicao,
