...
/* -----------------------------------------------------------------------------
   ...
   Confirma que round responde como esperado neste workspace, antes que
   qualquer consulta pesada seja executada.

   Todas as chamadas de round deste arquivo seguem o mesmo formato: o primeiro
   argumento é convertido explicitamente para ponto flutuante e o número de
   casas decimais é informado como literal inteiro. A conversão explícita
   mantém o tipo de saída uniforme e evita que expressões que misturam
   contagens inteiras com literais decimais produzam tipos de precisão fixa,
   cuja faixa pode ser insuficiente em agregações sobre centenas de milhões de
   linhas.

   O motor exige que o número de casas seja constante em tempo de análise. Uma
   coluna, um parâmetro de função ou qualquer expressão que dependa de linha
   nessa posição faz a consulta falhar informando que a escala precisa ser uma
   expressão inteira constante. Em particular, encapsular round dentro de uma
   função definida pelo usuário que receba as casas decimais como argumento
   reproduz exatamente esse erro, porque o argumento deixa de ser constante.

   Valores esperados: 0,3333 depois 33,33 e por fim 2,5.
----------------------------------------------------------------------------- */
SELECT
...
SELECT
  ...
  round(CAST(100.0 * wilson_inf(k_com, n_com) AS DOUBLE), 4)  AS ic95_inf_com_gr,
  round(CAST(100.0 * wilson_sup(k_com, n_com) AS DOUBLE), 4)  AS ic95_sup_com_gr,
  n_sem                                       AS envios_sem_gr,
  round(CAST(100.0 * p_sem AS DOUBLE), 4)                     AS taxa_sem_gr_pct,
  round(CAST(100.0 * wilson_inf(k_sem, n_sem) AS DOUBLE), 4)  AS ic95_inf_sem_gr,
  round(CAST(100.0 * wilson_sup(k_sem, n_sem) AS DOUBLE), 4)  AS ic95_sup_sem_gr,
...
SELECT
  ...
  round(CAST(100.0 * wilson_inf(entregues, envios) AS DOUBLE), 4)                    AS ic95_inf_pct,
  round(CAST(100.0 * wilson_sup(entregues, envios) AS DOUBLE), 4)                    AS ic95_sup_pct
FROM agg
;
...
SELECT
  ...
  round(CAST(100.0 * wilson_inf(k1, n1) AS DOUBLE), 4)             AS ic95_inf_concordante,
  round(CAST(100.0 * wilson_sup(k1, n1) AS DOUBLE), 4)             AS ic95_sup_concordante,
  n0                                               AS envios_discordante,
  k0                                               AS entregues_discordante,
  round(CAST(100.0 * p0 AS DOUBLE), 4)                             AS taxa_discordante_pct,
  round(CAST(100.0 * wilson_inf(k0, n0) AS DOUBLE), 4)             AS ic95_inf_discordante,
  round(CAST(100.0 * wilson_sup(k0, n0) AS DOUBLE), 4)             AS ic95_sup_discordante,
