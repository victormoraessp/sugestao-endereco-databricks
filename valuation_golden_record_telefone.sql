...
/* -----------------------------------------------------------------------------
   ...
   Estas duas funções são sempre chamadas em uma etapa própria da consulta, com
   o resultado nomeado, e nunca dentro de round. O motivo está documentado em
   01.5.
----------------------------------------------------------------------------- */
...
/* -----------------------------------------------------------------------------
   01.5  VERIFICAÇÃO DO COMPORTAMENTO DE ARREDONDAMENTO

   Confirma que round responde como esperado neste workspace, antes que qualquer
   consulta pesada seja executada. Valores esperados: 0,3333 depois 33,33 e por
   fim 2,5.

   REGRA SEGUIDA EM TODO O ARQUIVO
   Nenhuma chamada de round contém, no seu interior, uma função definida nas
   seções 01.1 a 01.3. Sempre que um intervalo de confiança precisa ser
   arredondado, a função é avaliada em uma etapa anterior, com resultado
   nomeado, e o arredondamento acontece na etapa seguinte.

   POR QUE ESSA SEPARAÇÃO É NECESSÁRIA
   O motor exige que o número de casas decimais de round seja constante no
   momento da análise da consulta. Quando uma função SQL definida pelo usuário
   aparece dentro da expressão arredondada, o processo de expansão dessa função
   reescreve a expressão inteira em uma projeção intermediária, e o literal das
   casas decimais deixa de ser reconhecido como constante. O resultado é a
   recusa da consulta com a indicação de que a escala precisa ser uma expressão
   inteira constante, mesmo quando o valor escrito no código é um número fixo.

   Avaliar a função antes e arredondar depois elimina o problema, porque nessa
   ordem o argumento de round já é uma coluna comum quando a análise ocorre.
----------------------------------------------------------------------------- */
SELECT
  round(CAST(1.0 / 3.0 AS DOUBLE), 4)         AS esperado_0_3333,
  round(CAST(100.0 * 1 / 3 AS DOUBLE), 2)     AS esperado_33_33,
  round(CAST(sum(v) / count(*) AS DOUBLE), 3) AS esperado_2_5
FROM (SELECT explode(array(1, 2, 3, 4)) AS v)
;
...
SELECT
  ...
  round(CAST(100.0 * avg(CASE WHEN tem_gr THEN 1 ELSE 0 END) AS DOUBLE), 4) AS pct_envios_com_gr,
  round(CAST(100.0 * avg(entregue) AS DOUBLE), 4)                           AS taxa_entrega_pct,
  round(CAST(avg(telefones_na_campanha) AS DOUBLE), 3)                      AS media_telefones_por_cpf_campanha
FROM vt_base_grupo
GROUP BY grupo
ORDER BY grupo
;
...
SELECT
  qualidade,
  count(*)                                            AS registros,
  round(CAST(100.0 * count(*) / sum(count(*)) OVER () AS DOUBLE), 4)  AS pct_registros
FROM vt_crm_envios_qualidade
GROUP BY qualidade
;
...
SELECT
  ...
  round(CAST(100.0 * avg(CASE WHEN tem_gr THEN 1 ELSE 0 END) AS DOUBLE), 4)  AS pct_com_gr
FROM vt_base_grupo
GROUP BY grupo
UNION ALL
SELECT
  ...
  round(CAST(100.0 * count(DISTINCT CASE WHEN tem_gr THEN cpf END)
              / nullif(count(DISTINCT cpf), 0) AS DOUBLE), 4)                 AS pct_com_gr
FROM vt_base_grupo
GROUP BY grupo
;
...
SELECT
  ...
  classe_concordancia,
  count(*)                                                        AS envios,
  round(CAST(100.0 * count(*) / sum(count(*)) OVER (PARTITION BY grupo) AS DOUBLE), 4) AS pct_envios,
...
SELECT
  ...
  round(CAST(100.0 * sum(n_envios) / sum(sum(n_envios)) OVER (PARTITION BY grupo) AS DOUBLE), 4) AS pct_envios,
  round(CAST(100.0 * sum(n_entregues) / nullif(sum(n_envios), 0) AS DOUBLE), 4)           AS taxa_entrega_pct,
  round(CAST(100.0 * sum(n_com_gr) / nullif(sum(n_envios), 0) AS DOUBLE), 4)              AS pct_com_gr,
...
intervalo AS (
  SELECT
    z.*,
    wilson_inf(z.k_com, z.n_com) AS ic_inf_com,
    wilson_sup(z.k_com, z.n_com) AS ic_sup_com,
    wilson_inf(z.k_sem, z.n_sem) AS ic_inf_sem,
    wilson_sup(z.k_sem, z.n_sem) AS ic_sup_sem
  FROM z
)
SELECT
  grupo,
  n_com                                       AS envios_com_gr,
  round(CAST(100.0 * p_com AS DOUBLE), 4)     AS taxa_com_gr_pct,
  round(CAST(100.0 * ic_inf_com AS DOUBLE), 4) AS ic95_inf_com_gr,
  round(CAST(100.0 * ic_sup_com AS DOUBLE), 4) AS ic95_sup_com_gr,
  n_sem                                       AS envios_sem_gr,
  round(CAST(100.0 * p_sem AS DOUBLE), 4)     AS taxa_sem_gr_pct,
  round(CAST(100.0 * ic_inf_sem AS DOUBLE), 4) AS ic95_inf_sem_gr,
  round(CAST(100.0 * ic_sup_sem AS DOUBLE), 4) AS ic95_sup_sem_gr,
  round(CAST(100.0 * (p_com - p_sem) AS DOUBLE), 4)           AS diferenca_pp,
  round(CAST(z_score AS DOUBLE), 3)                           AS z,
...
SELECT
  ...
  count(*)                                                                AS envios,
  round(CAST(100.0 * count(*) / sum(count(*)) OVER (PARTITION BY grupo) AS DOUBLE), 4)    AS pct
FROM vt_experimento
...
SELECT
  ...
  envios,
  entregues,
  round(CAST(100.0 * envios / sum(envios) OVER (PARTITION BY grupo) AS DOUBLE), 4) AS pct_envios,
  round(CAST(100.0 * entregues / nullif(envios, 0) AS DOUBLE), 4)                  AS taxa_entrega_pct,
  round(CAST(100.0 * ic_inf AS DOUBLE), 4)                                         AS ic95_inf_pct,
  round(CAST(100.0 * ic_sup AS DOUBLE), 4)                                         AS ic95_sup_pct
FROM intervalo
;
...
SELECT
  ...
  envios_melhor,
  envios_pior,
  round(CAST(100.0 * p_melhor AS DOUBLE), 4)                AS taxa_melhor_pct,
  round(CAST(100.0 * p_pior AS DOUBLE), 4)                  AS taxa_pior_pct,
  round(CAST(100.0 * (p_melhor - p_pior) AS DOUBLE), 4)     AS diferenca_pp,
  round(CAST((p_melhor - p_pior)
        / sqrt(p_pool * (1 - p_pool)
               * (1.0 / nullif(envios_melhor, 0) + 1.0 / nullif(envios_pior, 0))) AS DOUBLE), 3) AS z,
  p_valor_bilateral(
    (p_melhor - p_pior)
    / sqrt(p_pool * (1 - p_pool)
...
SELECT
  ...
  count(*)                                                          AS envios,
  round(CAST(100.0 * count(*) / sum(count(*)) OVER (PARTITION BY e.grupo) AS DOUBLE), 4) AS pct_envios,
  round(CAST(100.0 * avg(e.entregue) AS DOUBLE), 4)                                 AS taxa_entrega_pct
...
SELECT
  ...
  round(CAST(100.0 * p1 AS DOUBLE), 4)             AS taxa_concordante_pct,
  round(CAST(100.0 * ic_inf_conc AS DOUBLE), 4)    AS ic95_inf_concordante,
  round(CAST(100.0 * ic_sup_conc AS DOUBLE), 4)    AS ic95_sup_concordante,
  n0                                               AS envios_discordante,
  k0                                               AS entregues_discordante,
  round(CAST(100.0 * p0 AS DOUBLE), 4)             AS taxa_discordante_pct,
  round(CAST(100.0 * ic_inf_disc AS DOUBLE), 4)    AS ic95_inf_discordante,
  round(CAST(100.0 * ic_sup_disc AS DOUBLE), 4)    AS ic95_sup_discordante,
  round(CAST(100.0 * (p1 - p0) AS DOUBLE), 4)                      AS diferenca_pp,
  round(CAST(100.0 * (p1 - p0 - 1.959964 * se_diff) AS DOUBLE), 4) AS diferenca_ic95_inf_pp,
  round(CAST(100.0 * (p1 - p0 + 1.959964 * se_diff) AS DOUBLE), 4) AS diferenca_ic95_sup_pp,
  round(CAST(100.0 * (p1 - p0) / nullif(p0, 0) AS DOUBLE), 4)      AS lift_relativo_pct,
  round(CAST((k1 / nullif(n1 - k1, 0))
        / nullif(k0 / nullif(n0 - k0, 0), 0) AS DOUBLE), 4)   AS odds_ratio_bruta,
  round(CAST(z_score AS DOUBLE), 3)                                AS z,
  p_valor_bilateral(z_score)                       AS p_valor,
  CASE
...
SELECT
  ...
  round(CAST(100.0 * rd_mh AS DOUBLE), 4)                                       AS diferenca_pp_estratificada,
  round(CAST(or_mh AS DOUBLE), 4)                                               AS odds_ratio,
  round(CAST(exp(ln(or_mh) - 1.959964 * sqrt(var_ln_or)) AS DOUBLE), 4)         AS odds_ratio_ic95_inf,
  round(CAST(exp(ln(or_mh) + 1.959964 * sqrt(var_ln_or)) AS DOUBLE), 4)         AS odds_ratio_ic95_sup,
  round(CAST(chi2_cmh AS DOUBLE), 3)                                            AS qui2,
...
SELECT
  ...
  round(CAST(100.0 * sum(a + b + c + d)
        / sum(sum(a + b + c + d)) OVER (PARTITION BY grupo) AS DOUBLE), 4)     AS pct_envios
...
SELECT
  ...
  round(CAST(avg(p.n_conc) AS DOUBLE), 2)     AS media_envios_conc_por_cpf,
  round(CAST(avg(p.n_disc) AS DOUBLE), 2)     AS media_envios_disc_por_cpf,
  max(t.cpfs_no_grupo)        AS cpfs_no_experimento,
  round(CAST(100.0 * count(*) / nullif(max(t.cpfs_no_grupo), 0) AS DOUBLE), 4) AS pct_cpfs_pareados
...
SELECT
  ...
  nenhum_entregou,
  round(CAST(100.0 * taxa_gr AS DOUBLE), 4)                     AS taxa_telefone_gr_pct,
  round(CAST(100.0 * taxa_outro AS DOUBLE), 4)                  AS taxa_outro_telefone_pct,
  round(CAST(100.0 * (taxa_gr - taxa_outro) AS DOUBLE), 4)      AS diferenca_pp,
  round(CAST(b_so_gr / nullif(c_so_outro, 0) AS DOUBLE), 4)     AS razao_favoravel,
  round(CAST(chi2 AS DOUBLE), 3)                                AS qui2,
...
SELECT
  grupo,
  n                                                                    AS cpfs_pareados,
  round(CAST(100.0 * media_dif AS DOUBLE), 4)                                          AS diferenca_media_pp,
  round(CAST(100.0 * (media_dif - 1.959964 * dp_dif / sqrt(n)) AS DOUBLE), 4)          AS ic95_inf_pp,
  round(CAST(100.0 * (media_dif + 1.959964 * dp_dif / sqrt(n)) AS DOUBLE), 4)          AS ic95_sup_pp,
  round(CAST(media_dif / nullif(dp_dif / sqrt(n), 0) AS DOUBLE), 3)                    AS z,
...
SELECT
  ...
  count(*)                                                             AS envios,
  round(CAST(100.0 * count(*) / sum(count(*)) OVER (PARTITION BY grupo) AS DOUBLE), 4) AS pct_envios,
  round(CAST(100.0 * avg(entregue) AS DOUBLE), 4)                                      AS taxa_entrega_observada_pct
...
SELECT
  ...
  round(CAST(100.0 * avg(CASE WHEN classe_substituicao = 'PLANO_IGUAL_AO_USADO' THEN 1 ELSE 0 END) AS DOUBLE), 4) AS pct_plano_confirma,
  sum(CASE WHEN classe_substituicao <> 'PLANO_IGUAL_AO_USADO' THEN 1 ELSE 0 END) AS plano_troca,
  round(CAST(100.0 * avg(CASE WHEN origem_plano = 'RESERVA_RANKING_1' THEN 1 ELSE 0 END) AS DOUBLE), 4) AS pct_uso_de_reserva
FROM vt_plano_substituicao
GROUP BY grupo,
...
SELECT
  ...
  nenhum_entregou,
  round(CAST(100.0 * taxa_plano AS DOUBLE), 4)                   AS taxa_plano_pct,
  round(CAST(100.0 * taxa_usado AS DOUBLE), 4)                   AS taxa_telefone_usado_pct,
  round(CAST(100.0 * (taxa_plano - taxa_usado) AS DOUBLE), 4)    AS diferenca_pp,
  round(CAST(chi2 AS DOUBLE), 3)                                 AS qui2,
  p_valor_qui2_1gl(chi2)                         AS p_valor,
  CASE
...
SELECT
  ...
  round(CAST(avg(economia_disparos) AS DOUBLE), 4)                                  AS economia_media_disparos,
  round(CAST(100.0 * sum(economia_disparos)
        / nullif(sum(disparos_crm), 0) AS DOUBLE), 4)                          AS pct_reducao_disparos,
  ...
  round(CAST(100.0 * avg(CASE WHEN desfecho_comparado = 'SOMENTE_CRM_ENTREGA' THEN 1 ELSE 0 END) AS DOUBLE), 4) AS pct_perda_de_cobertura,
...
SELECT
  ...
  round(CAST(100.0 * avg(CASE WHEN desfecho_comparado = 'SOMENTE_CRM_ENTREGA' THEN 1 ELSE 0 END) AS DOUBLE), 4) AS pct_perda,
  round(CAST(avg(economia_disparos) AS DOUBLE), 4)                                  AS economia_media_disparos
FROM vt_simulacao_sequencia
WHERE totalmente_observado
...
SELECT
  ...
  round(CAST(100.0 * avg(CASE WHEN totalmente_observado THEN 1 ELSE 0 END) AS DOUBLE), 4) AS pct_observados,
  ...
  round(CAST(100.0 * sum(CASE WHEN totalmente_observado THEN tentativas ELSE 0 END)
        / nullif(sum(tentativas), 0) AS DOUBLE), 4)                                  AS pct_disparos_observados
FROM vt_simulacao_sequencia
GROUP BY grupo
;
...
SELECT
  ...
  round(CAST(100.0 * sum(CASE WHEN classe_concordancia = 'DIFERENTE' THEN 1 ELSE 0 END)
        / nullif(count(*), 0) AS DOUBLE), 4)                                             AS pct_sobre_total,
  round(CAST(100.0 * sum(CASE WHEN classe_concordancia = 'DIFERENTE' THEN 1 ELSE 0 END)
        / nullif(sum(CASE WHEN tem_gr THEN 1 ELSE 0 END), 0) AS DOUBLE), 4)              AS pct_sobre_com_gr
FROM vt_base_grupo
WHERE entregue = 0
GROUP BY grupo
UNION ALL
SELECT
  ...
  round(CAST(100.0 * count(DISTINCT CASE WHEN classe_concordancia = 'DIFERENTE' THEN cpf END)
        / nullif(count(DISTINCT cpf), 0) AS DOUBLE), 4),
  round(CAST(100.0 * count(DISTINCT CASE WHEN classe_concordancia = 'DIFERENTE' THEN cpf END)
        / nullif(count(DISTINCT CASE WHEN tem_gr THEN cpf END), 0) AS DOUBLE), 4)
FROM vt_base_grupo
WHERE entregue = 0
GROUP BY grupo
;
...
SELECT
  ...
  round(CAST(100.0 * count(*) / sum(count(*)) OVER (PARTITION BY grupo) AS DOUBLE), 4) AS pct,
  count(DISTINCT cpf)                                                  AS cpfs,
  round(CAST(100.0 * avg(gr_taxa_entrega) AS DOUBLE), 4)                               AS taxa_media_do_telefone_gr_pct
...
SELECT
  ...
  round(CAST(100.0 * g.taxa_aplicada AS DOUBLE), 4)                       AS taxa_aplicada_pct,
  round(CAST(g.entregas_recuperadas AS DOUBLE), 0)                        AS entregas_recuperadas,
  p.sucessos_em_risco,
  round(CAST(p.entregas_perdidas AS DOUBLE), 0)                           AS entregas_perdidas,
  round(CAST(g.entregas_recuperadas - p.entregas_perdidas AS DOUBLE), 0)  AS entregas_liquidas,
  round(CAST(100.0 * (g.entregas_recuperadas - p.entregas_perdidas)
        / nullif(v.envios_no_grupo, 0) AS DOUBLE), 4)                AS pp_incremental,
  v.envios_no_grupo
...
SELECT
  ...
  round(CAST(100.0 * avg(CASE WHEN alguma_entrega = 0 THEN 1 ELSE 0 END) AS DOUBLE), 4)             AS pct_sem_entrega,
...
SELECT
  ...
  round(CAST(100.0 * avg(b.entregue) AS DOUBLE), 4)                               AS taxa_entrega_pct,
  round(CAST(count(*) * max(gp.custo_sms) AS DOUBLE), 2)                          AS custo_total_disparos,
  round(CAST((count(*) - sum(b.entregue)) * max(gp.custo_sms) AS DOUBLE), 2)      AS custo_em_insucesso,
  round(CAST(100.0 * (count(*) - sum(b.entregue)) / nullif(count(*), 0) AS DOUBLE), 4) AS pct_custo_desperdicado,
  round(CAST(sum(b.entregue) * max(gp.taxa_conversao) AS DOUBLE), 2)              AS conversoes_estimadas,
  round(CAST(sum(b.entregue) * max(gp.taxa_conversao) * max(gp.valor_conversao) AS DOUBLE), 2) AS receita_estimada,
...
SELECT
  ...
  round(CAST(s.economia_disparos * gp.custo_sms AS DOUBLE), 2)                           AS economia_observada_reais,
  a.pct_disparos_observados,
  round(CAST(s.economia_disparos / nullif(a.pct_disparos_observados / 100.0, 0) AS DOUBLE), 0) AS economia_projetada_disparos,
  round(CAST(s.economia_disparos / nullif(a.pct_disparos_observados / 100.0, 0)
        * gp.custo_sms AS DOUBLE), 2)                                               AS economia_projetada_reais,
...
SELECT
  ...
  gp.valor_conversao,
  gp.custo_sms,
  round(CAST(s.entregas_liquidas * gp.taxa_conversao AS DOUBLE), 2)                          AS conversoes_incrementais,
  round(CAST(s.entregas_liquidas * gp.taxa_conversao * gp.valor_conversao AS DOUBLE), 2)     AS receita_incremental,
  round(CAST(s.entregas_liquidas * gp.custo_sms AS DOUBLE), 2)                               AS midia_requalificada
...
SELECT
  ...
  round(CAST(r.receita_incremental + e.economia_observada_reais AS DOUBLE), 2)         AS valor_total,
  r.midia_requalificada,
  b.custo_total_disparos,
  round(CAST(100.0 * (r.receita_incremental + e.economia_observada_reais)
        / nullif(b.custo_total_disparos, 0) AS DOUBLE), 4)                        AS valor_sobre_custo_pct,
  round(CAST((r.receita_incremental + e.economia_observada_reais)
        / nullif(b.disparos, 0) AS DOUBLE), 6)                                    AS valor_por_disparo,
  round(CAST((r.receita_incremental + e.economia_observada_reais)
        / nullif(b.entregas, 0) AS DOUBLE), 6)                                    AS valor_por_entrega_atual
...
SELECT
  ...
  s.rotulo_taxa,
  s.rotulo_valor,
  round(CAST(r.taxa_conversao * s.mult_taxa_conversao AS DOUBLE), 6)                 AS taxa_conversao_aplicada,
  round(CAST(r.valor_conversao * s.mult_valor_conversao AS DOUBLE), 2)               AS valor_conversao_aplicado,
  r.entregas_liquidas,
  round(CAST(r.entregas_liquidas * r.taxa_conversao * s.mult_taxa_conversao AS DOUBLE), 2) AS conversoes_incrementais,
  round(CAST(r.entregas_liquidas * r.taxa_conversao * s.mult_taxa_conversao
        * r.valor_conversao * s.mult_valor_conversao AS DOUBLE), 2)             AS receita_incremental,
  e.economia_observada_reais,
  round(CAST(r.entregas_liquidas * r.taxa_conversao * s.mult_taxa_conversao
        * r.valor_conversao * s.mult_valor_conversao
        + e.economia_observada_reais AS DOUBLE), 2)                             AS valor_total
FROM vt_res_09_receita r
...
SELECT
  ...
  v.faixa_volume,
  v.n_envios,
  round(CAST(100.0 * v.taxa_entrega AS DOUBLE), 4)                                        AS taxa_entrega_pct,
  v.n_envios - v.n_entregues                                              AS insucessos,
  coalesce(d.insucessos_divergentes, 0)                                   AS insucessos_divergentes,
  round(CAST(100.0 * d.taxa_calibracao AS DOUBLE), 4)                                     AS taxa_calibracao_pct,
  round(CAST(coalesce(d.insucessos_divergentes, 0) * d.taxa_calibracao AS DOUBLE), 1)     AS recuperadas,
  round(CAST(coalesce(d.sucessos_em_risco, 0) * (1 - d.taxa_calibracao) AS DOUBLE), 1)    AS perdidas,
  round(CAST(coalesce(d.insucessos_divergentes, 0) * d.taxa_calibracao
        - coalesce(d.sucessos_em_risco, 0) * (1 - d.taxa_calibracao) AS DOUBLE), 1)  AS entregas_liquidas,
  coalesce(d.recuperacao_comprovada, 0)                                   AS recuperacao_comprovada,
  round(CAST((coalesce(d.insucessos_divergentes, 0) * d.taxa_calibracao
         - coalesce(d.sucessos_em_risco, 0) * (1 - d.taxa_calibracao))
        * gp.taxa_conversao * gp.valor_conversao AS DOUBLE), 2)                      AS receita_incremental,
  round(CAST(v.n_envios * gp.custo_sms AS DOUBLE), 2)                                     AS custo_disparos_campanha
...
SELECT
  ...
  round(CAST(100.0 * z.p1 AS DOUBLE), 4)                 AS taxa_concordante_pct,
  CAST(z.n0 AS BIGINT)                   AS envios_discordante,
  round(CAST(100.0 * z.p0 AS DOUBLE), 4)                 AS taxa_discordante_pct,
  round(CAST(100.0 * (z.p1 - z.p0) AS DOUBLE), 4)        AS diferenca_pp,
  round(CAST(z.z_score AS DOUBLE), 3)                    AS z,
...
SELECT
  ...
  derrotas,
  empates,
  round(CAST(100.0 * vitorias / nullif(decididas, 0) AS DOUBLE), 4) AS pct_vitorias,
...
SELECT
  ...
  round(CAST(100.0 * p0 AS DOUBLE), 4)            AS taxa_discordante_pct,
  round(CAST(100.0 * (p1 - p0) AS DOUBLE), 4)     AS diferenca_pp,
  round(CAST((p1 - p0) / sqrt(p_pool * (1 - p_pool)
        * (1.0 / nullif(n1, 0) + 1.0 / nullif(n0, 0))) AS DOUBLE), 3) AS z,
...
SELECT
  grupo,
  'EXATA' AS regra,
  n1 AS envios_concordante, round(CAST(100.0 * p1 AS DOUBLE), 4) AS taxa_concordante_pct,
  n0 AS envios_discordante, round(CAST(100.0 * p0 AS DOUBLE), 4) AS taxa_discordante_pct,
  round(CAST(100.0 * (p1 - p0) AS DOUBLE), 4) AS diferenca_pp,
  p_valor_bilateral((p1 - p0) / sqrt(p_pool * (1 - p_pool)
        * (1.0 / nullif(n1, 0) + 1.0 / nullif(n0, 0)))) AS p_valor
FROM calc
