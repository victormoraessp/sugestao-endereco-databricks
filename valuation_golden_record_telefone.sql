...
/* -----------------------------------------------------------------------------
   ...
   Define até três recortes de campanhas que são analisados em paralelo. Cada
   grupo produz sua própria linha em todas as tabelas de resultado, e o rótulo
   TOTAL consolida o conjunto.

   Regra de escopo:
     Se ao menos um grupo tiver campanhas listadas, a análise roda apenas sobre
     as campanhas listadas, e TOTAL é a união delas.
     Se os três grupos estiverem vazios, a análise roda sobre a base completa,
     sob o rótulo TOTAL.
   ...
   Preenchimento da coluna campanhas:
     array('NOME_DA_CAMPANHA_A', 'NOME_DA_CAMPANHA_B')
...
SELECT
  ...
  CAST(array() AS ARRAY<STRING>)     AS campanhas,
...
/* -----------------------------------------------------------------------------
   02.1  FONTES DE DADOS
----------------------------------------------------------------------------- */
CREATE OR REPLACE TEMP VIEW vt_fonte_golden_record AS
SELECT * FROM temp_view_golden_record
;
...
/* -----------------------------------------------------------------------------
   ...
   Materializa a regra descrita em 00.2. Cada campanha recebe os rótulos de
   grupo a que pertence, mais o rótulo TOTAL.
----------------------------------------------------------------------------- */
CREATE OR REPLACE TEMP VIEW vt_grupo_campanha AS
WITH definido AS (
  SELECT grupo, explode(campanhas) AS campanha
  FROM vt_param_grupos
  WHERE size(campanhas) > 0
),
contagem AS (
...
SELECT
  ...
  p.custo_sms
FROM vt_param_grupos g
CROSS JOIN vt_param p
WHERE size(g.campanhas) > 0
UNION ALL
SELECT
  'TOTAL',
