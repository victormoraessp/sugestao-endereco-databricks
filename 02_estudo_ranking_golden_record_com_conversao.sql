...
universo AS (SELECT DISTINCT id_briefing FROM tb_crm_envio_analitico),
total_sem_lista AS (
  SELECT 'TOTAL' AS grupo, u.id_briefing
  FROM universo u CROSS JOIN contagem c WHERE c.listadas = 0
),
total_com_lista AS (SELECT DISTINCT 'TOTAL' AS grupo, id_briefing FROM definido)
...
/* -----------------------------------------------------------------------------
   ...
   A coluna base_de_calculo nomeia o denominador de cada indicador. Sem ela, os
   percentuais deste resumo seriam lidos como se pertencessem a uma escala
   unica, quando na verdade incidem sobre recortes diferentes: ora todos os
   disparos, ora apenas os de primeira posicao, ora apenas o conjunto em que a
   simulacao consegue observar os dois desfechos.
----------------------------------------------------------------------------- */
SELECT
  grupo,
  bloco,
  ...
  unidade,
  CASE ordem
    WHEN  1 THEN 'contagem absoluta, sem denominador'
    WHEN  2 THEN 'media por cliente com cobertura'
    WHEN  3 THEN 'todos os disparos do escopo'
    WHEN 10 THEN 'disparos cujo numero ocupa a primeira posicao do ranking'
    WHEN 11 THEN 'disparos cujo numero ocupa a segunda posicao do ranking'
    WHEN 12 THEN 'disparos cujo numero nao consta em nenhuma posicao'
    WHEN 13 THEN 'diferenca entre a primeira e a segunda posicao'
    WHEN 14 THEN 'diferenca entre a primeira posicao e numeros fora do ranking'
    WHEN 20 THEN 'apenas os disparos de primeira posicao'
    WHEN 21 THEN 'apenas os disparos cujo telefone do plano tem desfecho observado'
    WHEN 22 THEN 'apenas os disparos de primeira posicao com desfecho observado'
    WHEN 23 THEN 'todos os disparos do escopo'
    WHEN 30 THEN 'todos os disparos do escopo'
    WHEN 31 THEN 'disparos ate o desfecho, no conjunto integralmente observado'
    WHEN 32 THEN 'contagem absoluta, sem denominador'
    WHEN 33 THEN 'contagem absoluta, sem denominador'
    WHEN 34 THEN 'pares de cliente e campanha do conjunto observado'
    WHEN 40 THEN 'contagem absoluta, sem denominador'
    WHEN 41 THEN 'disparos poupados no conjunto observado'
    WHEN 42 THEN 'extrapolacao para todos os disparos do escopo'
